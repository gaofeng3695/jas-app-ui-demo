/**
 * @author gf 2018.09.18
 * @description f2 图表 通用方法
 *
 */
var f2Tools = (function () {
  var obj = {
    addPieLabel: function (chart, fnTextFormat) {
      // @ example
      // f2Tools.addPieLabel(chart, function (_data) {
      //   return _data.name + '\n' + (_data.percent / total * 100).toFixed(2) + '%(' + _data.percent + ')';
      // });
      ...
      function createLabelText(_data) {
        return _data.name + '\n' + (_data.percent / total * 100).toFixed(2) + '%(' + _data.percent + ')';
      }

      // 绘制文本以及连接线
      function drawLabel(label, createLabelText) {
